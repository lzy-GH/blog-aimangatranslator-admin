    ...
    >
      <template v-slot:button>
        <a-upload
          ...
          name="blogFile"
          :action="`/api/mangaBlog/uploadMangaBlog2ProdEnv`"
          :headers="headers"
          @change="handleChange"
        >
      ...
      <template v-slot:content>
        <div class="blog-img">
          ...
          <a-upload
            ...
            @change="handleChange"
          >
            <!-- <a-button @click="addBlo" type="primary">添加</a-button> -->
          </a-upload>
          <ul class="imglist">
            <li v-for="(item, index) in imglistinfo" :key="index" class="item">
              ...
              <div class="btn">
                <!-- <a-button @click="addBlo" type="primary">替换</a-button> -->
              </div>
            </li>
          </ul>
        </div>
...
import { message, Modal } from "ant-design-vue"; //提示组件
import {
  IListOperation,
  ISearchItems,
  ShowClient,
...
function funBtn(key: string, item?: any): void {
  ...
  if (key == "del") {
    Modal.confirm({
      ...
      onOk() {
        delProdJson(item.name).then((res) => {
          console.log("delProdJson", res);
          if (res.code == 200) {
  ...
  if (key == "dow") {
    downLoadProdJson(item.name).then((res) => {
      console.log("downLoadProdJson", res);
      downLoadProdJsonFile(res, item.name);
    });
  }
}
// 下载json文件
function downLoadProdJsonFile(data: any, fileName: string) {
  // 将数据转换为 JSON 字符串
...
// 更新线上
let RestartLoading = ref<boolean>(false);
function newProd() {
  Modal.confirm({
    title: () => "是否确定执行操作?",
    icon: () => createVNode(ExclamationCircleOutlined),
